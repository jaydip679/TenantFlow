...
const computeSignals = async (tenantId) => {
  ...
  const [totalInvoices, paidInvoices] = await Promise.all([
    ReadInvoice.countDocuments({
      tenantId,
      invoiceCreatedAt: { $gte: sixMonthsAgo },
  ...
  const sub = await ReadSubscription.findOne({
    tenantId,
  ...
  if (sub?.subscriptionCreatedAt) {
    const ageDays = differenceInDays(now, new Date(sub.subscriptionCreatedAt));
  ...
  const paidInvoicesWithDates = await ReadInvoice.find({
    tenantId,
    status:    'paid',
    paidAt:    { $exists: true, $ne: null },
    invoiceCreatedAt: { $gte: sixMonthsAgo },
  }).select('invoiceCreatedAt paidAt').lean();
  ...
  if (paidInvoicesWithDates.length > 0) {
    const totalDays = paidInvoicesWithDates.reduce((sum, inv) => {
      return sum + Math.max(0, differenceInDays(new Date(inv.paidAt), new Date(inv.invoiceCreatedAt)));
    }, 0);
  ...
  // ── 5. Dunning Risk (10%) ──────────────────────────────────────────────────
  const activeDunning = await ReadDunningRecord.findOne({
    tenantId,
    status: 'active',
  }).lean();
